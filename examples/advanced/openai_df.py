...
TranslationPipeline + OpenAIProvider from dgp.

Mirrors translate_gemini.py exactly — only the provider and defaults differ.
"""
...
from dgp.providers import OpenAIProvider
...
def _translate_chunk(
    ...
    OpenAI-specific retry notes:
      - RateLimitError (429) → back off and retry
      - APIConnectionError   → retry
      - AuthenticationError  → fail immediately (no point retrying)
    """
    last_error = None
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            # Authentication errors won't recover on retry — surface immediately
            if "authentication" in err_str or "invalid api key" in err_str:
                raise RuntimeError(
            ...
            if attempt < MAX_RETRIES:
                ...
                delay = RETRY_DELAY * attempt * (3 if "rate" in err_str else 1)
                time.sleep(delay)
...
def translate_language_df(
    ...
    print(f"\n[OpenAI] [{src_language} → {tgt_language}]  {total:,} rows to translate  "
...
def run_pipeline(
    ...
    # ── Build one shared pipeline (provider reads OPENAI_API_KEY from env) ─
    provider = OpenAIProvider()  # reads OPENAI_API_KEY automatically
    config   = ModelConfig(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Translate a dataset (English ↔ Kinyarwanda) using OpenAI GPT-5.",
    ...
    parser.add_argument("--max-tokens",  type=int, default=4096, help="max_tokens for OpenAI response")
    parser.add_argument("--concurrency", type=int, default=8,    help="Max parallel OpenAI requests")
...
def main() -> None:
    ...
    if not os.getenv("OPENAI_API_KEY"):
        raise EnvironmentError(
            ...
            "Export it before running:  export OPENAI_API_KEY='your-key-here'"
        )
