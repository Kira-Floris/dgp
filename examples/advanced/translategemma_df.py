...
def load_nllb(model_name: str, device: torch.device):
    from transformers import AutoModelForSeq2SeqLM, AutoTokenizer
    print(f"\nLoading NLLB model {model_name!r} …")
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    # CHANGED: use device_map="auto" to spread across all available GPUs
    model = AutoModelForSeq2SeqLM.from_pretrained(
        model_name,
        torch_dtype=torch.float16,
        device_map="auto",           # was: .to(device) — now uses both GPUs
        low_cpu_mem_usage=True,
    )
    model.eval()
    print("Model ready.\n")
...
def load_translategemma(model_name: str, device: torch.device):
    from transformers import AutoModelForImageTextToText, AutoProcessor
    print(f"\nLoading TranslateGemma model {model_name!r} …")
    processor = AutoProcessor.from_pretrained(model_name)
    model = AutoModelForImageTextToText.from_pretrained(
        model_name,
...
def translate_language_df_nllb(
    ...
    # CHANGED: sort by text length to minimise padding waste within each batch
    paired   = sorted(zip(indices, texts), key=lambda x: len(x[1]))
    indices, texts = map(list, zip(*paired)) if paired else ([], [])
    ...
    for i in range(0, total, batch_size):
        ...
        # CHANGED: removed .to(device) — model.device handles placement with device_map="auto"
        inputs = tokenizer(
            batch_texts,
            return_tensors="pt",
            padding=True,
        ...
        # CHANGED: cap max_new_tokens dynamically per batch based on input length
        batch_max_tokens = min(max_new_tokens, inputs["input_ids"].shape[1] * 2)
    ...
    # CHANGED: restore original index order after length-sorted translation
    order = {idx: pos for pos, idx in enumerate(indices)}
    sub_df["translation_text"] = [translations[order[i]] for i in sub_df.index]
    return sub_df
...
def translate_language_df_gemma(
    ...
    # Left-padding keeps all sequences aligned at the right (end of prompt)
    processor.tokenizer.padding_side = "left"
    ...
    for i in range(0, total, batch_size):
        ...
        # CHANGED: single batched apply_chat_template call with padding=True
        # replaces the old per-row loop + manual padding tensors
        inputs = processor.apply_chat_template(
            ...
            padding=True,
            truncation=True,
        ).to(model.device)   # CHANGED: model.device instead of device arg

        prompt_len = inputs["input_ids"].shape[1]

        # CHANGED: cap max_new_tokens dynamically per batch
        batch_max_tokens = min(max_new_tokens, prompt_len * 2)
        ...
        # CHANGED: slice and batch_decode all rows at once instead of a per-row loop
        new_tokens = outputs[:, prompt_len:]
        translations.extend(processor.batch_decode(new_tokens, skip_special_tokens=True))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--batch-size",     type=int, default=32,  help="Rows per GPU batch")
    parser.add_argument("--max-new-tokens", type=int, default=512, help="Max tokens to generate per row")
    parser.add_argument("--device",         type=int, default=0,   help="GPU index (ignored when device_map=auto)")
    parser.add_argument("--model",          default=DEFAULT_MODEL, help="NLLB or TranslateGemma model name")
    parser.add_argument("--format",         default="csv",         choices=["parquet", "csv", "json"],
                                            help="Output format")
    return parser.parse_args()
...
def main() -> None:
    ...
    print(f"Device : {device}  (device_map=auto will use all available GPUs)")
    print(f"Model  : {args.model}  ({'TranslateGemma' if is_translategemma(args.model) else 'NLLB'})")
    print(f"Input  : {args.input}")
    print(f"Output : {args.output}  [{args.format}]")
