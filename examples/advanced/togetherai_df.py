"""
translate_together.py
---------------------
Translates a bilingual (English ↔ Kinyarwanda) dataset using the
TranslationPipeline + TogetherAIProvider from dgp.

Mirrors translate_openai.py and translate_gemini.py exactly.
API key is read exclusively from the TOGETHER_API_KEY environment variable.
"""
...
from dgp.providers import TogetherAIProvider
...
def _translate_chunk(
    ...
    TogetherAI-specific retry notes:
      - RateLimitError (429) → back off longer and retry
      - AuthenticationError  → fail immediately (bad key won't recover)
      - ServerError (5xx)    → retry normally
    """
    last_error = None
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            # Authentication errors won't recover — surface immediately
            if "authentication" in err_str or "invalid api key" in err_str or "401" in err_str:
                raise RuntimeError(
                    f"TogetherAI authentication failed — check your TOGETHER_API_KEY. Error: {e}"
                ) from e

            if attempt < MAX_RETRIES:
                # Longer back-off for rate-limit errors
                delay = RETRY_DELAY * attempt * (3 if "rate" in err_str or "429" in err_str else 1)
                time.sleep(delay)
...
def translate_language_df(
    ...
    print(f"\n[TogetherAI] [{src_language} → {tgt_language}]  {total:,} rows to translate  "
...
def run_pipeline(
    ...
    # ── Build one shared pipeline (provider reads TOGETHER_API_KEY from env)
    provider = TogetherAIProvider()  # reads TOGETHER_API_KEY automatically
    config   = ModelConfig(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Translate a dataset (English ↔ Kinyarwanda) using TogetherAI.",
    ...
    parser.add_argument("--model",       default=DEFAULT_MODEL,  help="TogetherAI model name")
    parser.add_argument("--max-tokens",  type=int, default=2048, help="max_tokens for TogetherAI response")
    parser.add_argument("--concurrency", type=int, default=8,    help="Max parallel TogetherAI requests")
...
def main() -> None:
    ...
    if not os.getenv("TOGETHER_API_KEY"):
        raise EnvironmentError(
            ...
            "Export it before running:  export TOGETHER_API_KEY='your-key-here'"
        )
