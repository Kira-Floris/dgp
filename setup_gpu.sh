...
# OPTIMIZED RUN
# sudo docker run -d --runtime nvidia --gpus all -v ~/.cache/huggingface:/root/.cache/huggingface --env "HF_TOKEN=$HF_TOKEN" -p 10000:8000 --ipc=host --shm-size=10g vllm/vllm-openai:latest --model openai/gpt-oss-120b --tensor-parallel-size 8 --max-model-len 1024 --max-num-seqs 256 --gpu-memory-utilization 0.60 --enforce-eager --disable-log-requests --max-num-batched-tokens 1024 --block-size 16 --swap-space 4
# sudo docker run -d --runtime nvidia --gpus all -v ~/.cache/huggingface:/root/.cache/huggingface --env "HF_TOKEN=$HF_TOKEN" -p 10000:8000 --ipc=host --shm-size=10g vllm/vllm-openai:latest --model openai/gpt-oss-120b --tensor-parallel-size 8 --max-model-len 2048 --max-num-seqs 2048 --gpu-memory-utilization 0.60 --disable-log-requests --max-num-batched-tokens 2048 --block-size 16 --swap-space 4
